...
class MultiCategoricalDistribution(Distribution):
    ...
    def proba_distribution(self, action_logits: th.Tensor) -> "MultiCategoricalDistribution":
        self.distribution = [Categorical(logits=split, validate_args=False) for split in th.split(action_logits, tuple(self.action_dims), dim=1)]
        return self
...
class StateDependentNoiseDistribution(Distribution):
    ...
    def get_std(self, log_std: th.Tensor) -> th.Tensor:
        ...
        return th.ones(self.latent_sde_dim, self.action_dim, device=log_std.device) * std
